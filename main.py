...
class App:
    ...
    def choose_date_time(self):
        ...
        date_label = tk.Label(frame, text="Выберите дату", font=("Arial", 16))
        date_label.pack(pady=10)

        calendar = Calendar(frame, date_pattern='yyyy-mm-dd')
        calendar.pack(pady=5)

        time_label = tk.Label(frame, text="Выберите время", font=("Arial", 16))
        ...
        for i, time in enumerate(available_times):
            ...
            tk.Button(time_frame, text=time, font=("Arial", 12),
                      command=lambda t=time: check_availability(t)).grid(
        ...
        def select_time(selected_time):
            self.selected_date = calendar.get_date()
            self.selected_time = selected_time
            self.enter_patient_details()

        for time in available_times:
            tk.Button(time_frame, text=time, font=("Arial", 12), command=lambda t=time: select_time(t)).pack(
    ...
    def enter_patient_details(self):
        ...
        fio_label = tk.Label(frame, text="Введите ФИО", font=("Arial", 16))
        fio_label.pack(pady=10)

        fio_entry = tk.Entry(frame, font=("Arial", 14))
        fio_entry.pack(pady=5)

        phone_label = tk.Label(frame, text="Введите номер телефона", font=("Arial", 16))
        phone_label.pack(pady=10)

        phone_entry = tk.Entry(frame, font=("Arial", 14))
        phone_entry.pack(pady=5)

        birthday_label = tk.Label(frame, text="Выберите дату рождения", font=("Arial", 16))
...
Телефон: {phone}"""
        ...
        tk.Label(frame, text=summary, font=("Arial", 14), justify=tk.LEFT).pack(pady=10)
            ...
            try:
                with conn.cursor() as cursor:
                    ...
                    if appointments:
                        ...
                        messagebox.showwarning("Нет пациента", "Пациента нет")
            except pymysql.MySQLError as e:
    ...
    def display_appointments(self, appointments):
        ...
        for appointment in appointments:
            tk.Label(frame, text=f"{appointment[0]} - {appointment[1]} - {appointment[2]}", font=("Arial", 14)).pack(pady=5)
